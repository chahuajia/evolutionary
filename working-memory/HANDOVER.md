...
**写于**：2026-09-20 14:23（v45；波次以 `loop.md` 为准）
**读者**：新开对话的 Agent（不是给用户的作业）
**取代**：v44（仍写 HEAD `5d71df5` —— 已过时）

**当前总状态**：**▶ 运行中（主树父写）**。HEAD `cad4c22`。前端 153/153。
wave121 已落。常设规则：父自选推进，不等人。
...
见 `tasks/evo-collab-extreme/loop.md`。前端 **153/153** · HEAD `cad4c22`。父自选推进。
门禁轴可 idle；wave121 loadMallSku。下刀 loadMerchantProfile / 领券岛经 loadCampaign。拒 ReferralBinding/CreditLedgerDebt。
