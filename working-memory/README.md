# Working Memory — evolutionary（换电平台）

**更新**：2026-09-17 ｜ HEAD `a4e9f4f`

## 仓库归属

| 本仓 | 其他仓 |
| :--- | :--- |
| 前后端实现 · 业务域压测规格 | collab-cli = 工具链 · collaboration = 长期 KB |

## 活跃任务

| 任务 | 状态 | 文件 |
| :--- | :--- | :--- |
| **battery-phase-1 实现** | 🔄 切片 1 ✅ · 切片 2 集群中 | `tasks/battery-phase-1/loop.md` |
| **battery-phase-0 实现** | ✅ 切片 1–4 收口 · 68 tests | `tasks/battery-phase-0/retro.md` |
| **battery-pressure** | ✅ 规格 0–7 收口 | `tasks/battery-pressure/retro.md` |
| evolutionary KB 压测 | ✅ 4–15 轮 | `../specs/pressure-test-retro.md` |

## 知识库

`D:\actto\front\project\collaboration_aggregate\collaboration`

## 陷阱

AI 不 push
