...
**HEAD**：`374d5fd`（ahead ~47；enum 内聚 refactor **未 commit**）  
**idle**：2 / 3 · **lanes**：0
...
## 本地 refactor（未 commit）

- **enum 内聚**：20 个单聚合 `*Status` → `Aggregate.Status` 内嵌；**保留** `BatteryStatus`（跨 battery/station/swap）
- 工具：`backend/tools/nest-single-aggregate-enums.mjs`
